...
export function MeasureTool( viewer, options, sharedMeasureConfig, snapper )
{
    ...
    // ROMCOCA
    this.onClickManual = function(e) {

        console.log('onClickManual =-------------- ', e)

        const vector = new THREE.Vector3(e.detail.x, e.detail.y, 0);
        const canvas = _viewer.canvas; // `renderer` is a THREE.WebGLRenderer

        // obj.updateMatrixWorld();  // `obj´ is a THREE.Object3D
        // vector.setFromMatrixPosition(obj.matrixWorld);

        vector.project(_viewer.getCamera()); // `camera` is a THREE.PerspectiveCamera
        const xRender = Math.round((0.5 + vector.x / 2) * (canvas.width / window.devicePixelRatio));
        const yRender = Math.round((0.5 - vector.y / 2) * (canvas.height / window.devicePixelRatio));

        _measurementType = 3;
        _consumeSingleClick = true;

        _cursorPosition = MeasureCommon.inverseProject({ x: xRender, y: yRender}, _viewer);

        console.log('xRender -------------- ', xRender)
        console.log('yRender -------------- ', yRender)

        this.handleSingleClick({
            startManual: true,
            isManual: true,
            MarkupID: e.detail?.MarkupID
        }, null)
    }

    // ROMCOCA
    this.onClickClearManual = function() {
        _measurementType = 3;
        _consumeSingleClick = true;
        _cursorPosition = MeasureCommon.inverseProject({ x: 0, y: 0}, _viewer);
        this.handleSingleClick({
            startManual: true,
            isManual: true,
            MarkupID: 0
    ...
    this.onRemoveMeasurement = function(e) {
        var MarkupID = e.detail;
            ...
            console.log('removing MarkupID / itemid', MarkupID, item);
    ...
    this.activate = function()
    {
       ...
       this.onClickManualBinded = this.onClickManual.bind(this);
       ...
       _viewer.addEventListener("clickManual", this.onClickManualBinded);
    ...
    this.deactivate = function()
    {
        ...
        _viewer.removeEventListener("clickManual", this.onClickManualBinded);
    };
    ...
    function snapToFirstPick(currentPick, forceSnap) {
        console.log('Check ---------------- snapToFirstPick')
    ...
    this._handleMouseEvent = function (event) {
        ...
        if (_snapper.isSnapped() || event?.startManual) {
            console.log('check ------------------------ ok 1111')
            ...
            var resultPick = getActivePick();
            _snapper.copyResults(resultPick);
            valid = true;
        } 
        else { 
    ...
    this._doConsumeSingleClick = function() {
        console.log('check ----------- _doConsumeSingleClick')
        ...
        if (av.isTouchDevice()) {
            switch( event.type )
            {
                case "press":
                    ...
                    console.log('cjeck ----------- handlePressHold')
                    this._handleMouseEvent(event);
                    _snapper.indicator.render();
    ...
    this.handleButtonUp = function (event, button) {
        console.log('check ------------------ handleButtonUp')
        _isDragging = false;
        _downX = null;
        _downY = null;
        
        if (_endpointMoved) {
            console.log('check ------------------ handleButtonUp trueeee')
            _consumeSingleClick = true;
            _singleClickHandled = !_singleClickHandled;
            this.handleSingleClick(event);
    ...
    this.handleSingleClick = function (event, button) {
        ...
        if (_consumeSingleClick) {
            console.log('event handle mouse:', event)
            ...
            if (_currentMeasurement && _currentMeasurement.isComplete()) {
                if (event.isManual) {
                    _currentMeasurement.MarkupID = event?.MarkupID;
                    _currentMeasurement.roomName = 'Id: ' + event?.MarkupID;
                    this.setValueMeasurementLabelText(_currentMeasurement.indicator.areaLabel.label, _currentMeasurement.roomName);
                    ...
                    var checkRoomId = _currentMeasurement?.roomName;
                    console.log('check -------------- ', checkRoomId)

                    if (checkRoomId && !checkRoomId.includes('Id: ')) {
                        _currentMeasurement.roomName = 'Id: --';
                        this.setValueMeasurementLabelText(_currentMeasurement.indicator.areaLabel.label, _currentMeasurement.roomName);
                    }

                    if (_measurementsManager) {
                        if (checkRoomId && (!checkRoomId.includes('Id: ') || (checkRoomId.includes('Id: ') && checkRoomId.replace('Id: ', '') == '--'))) {
                            ...
                            if (roomID != undefined) {
                                ...
                                _currentMeasurement.roomName = 'Id: ' + roomID;
                                this.setValueMeasurementLabelText(_currentMeasurement.indicator.areaLabel.label, _currentMeasurement.roomName);
                                var clone = {..._currentMeasurement};
                                delete(clone.indicator)
                                document.dispatchEvent(new CustomEvent('addNewRoom', {'detail': JSON.stringify(clone)}));
    ...
    this.onCameraChange = function () {
        ...
        console.log('cjeck ----------- onCameraChange')
    ...
    this.handleSingleTap = function (event) {
        if (!_singleClickHandled) {
            _consumeSingleClick = true;
            console.log('Check ---------------- handleSingleTap')
    ...
    this.handleDoubleTap = function(event) {
        console.log('Check ---------------- handleDoubleTap')
