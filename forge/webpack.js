...
module.exports = function(env) {
    ...
    const build_task = "lmv-extensions";
    // if (env.BUILD_TASK === 'lmv-all') {
    if (build_task === 'lmv-all') {
        ...
        var task = nameToTask[build_task];
        if (!task)
            // throw new Error(`Invalid env.BUILD_TASK value: (${env.BUILD_TASK})`);
            throw new Error(`Invalid env.BUILD_TASK value: (${build_task})`);
            exportTasks = [task];
    }
